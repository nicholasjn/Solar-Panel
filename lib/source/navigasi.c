...
roomType roomStart(uint16_t time) {
	uint16_t front, left, right, back, length;
	uint8_t i, j, correct;
	const uint8_t ping[4] = {0xE0, 0xE4, 0xEC, 0xE8, 0xE6, 0xEA};
...
roomType checkRoomIn(){
	uint16_t front, left, right, back, length, width, temp, i, wide;
...
void approachFire(uint16_t tpa81Val) {
	...
	while(1) {
		if(stopSearch) {
			moveRobot(0, 5, moveSearchAngle, time);
			moveRobot(0, 5, 0, time);
			moveNormal(time);
			break;
		}
			...
			moveRobot(0, 3, -moveSearchAngle * 1.9, time);
		...
		moveRobotTPA(0, 3, -moveSearchAngle, time);
		moveFireCounter++;
	}
}
...
void room1toRoom2(uint16_t time){
	const uint8_t ping[2] = {0xE6, 0xEA, 0xE0};
	uint8_t left, right, front, i;
	sonarRange(ping[0]);
	right = sonarGet(ping[0]);
	sonarRange(ping[1]);
